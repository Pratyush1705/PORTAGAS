...
tool_icon= pyfiglet.figlet_format("PORTAGAS",font="slant")
print(tool_icon)
print("Creator: Pratyush Balivada")
print("Description of the tool: This tool is a simple Port scanner which does \nQuick scans, Detailed scans and various other scans and os detection \nbased on the user choice")
print("-"*70)
try:
   print("Enter Choice of input:\n1)URL Address\n2)IP Address")
   choice=input()
   if choice=='1':
        targetURL= input("Enter URL Address: ")
        IPAddr = socket.gethostbyname(targetURL) 
        nm = nmap.PortScanner()
        machine = nm.scan(IPAddr, arguments='-O')
        print("Entered URL is:" + targetURL)    
        print("IP Address:" + IPAddr)    
        print("OS: ")
        print(machine['scan'][IPAddr]['osmatch'][0]['osclass'][0]['osfamily'])
   elif choice=='2':
        IPAddr= input("Enter IP Address: ")
        URLaddr= socket.gethostbyaddr('{}'.format(IPAddr))
        nm = nmap.PortScanner()
        machine = nm.scan(IPAddr, arguments='-O')
        print("Entered IP : "+IPAddr)
        print("DNS Record:")
        print(URLaddr)
        print("OS: ")
        print(machine['scan'][IPAddr]['osmatch'][0]['osclass'][0]['osfamily'])
   ...
   print("Enter Type of Scan:\n1)Quick Scan (Scans Top 1000 ports)\n2)Detailed Scan (Scans all 65535 ports)\n3)Specific Port Scan\n4)Port Range Scan (Scans ports in specific interval)")
   scantype=input()
   t1=datetime.now()
   print("-"*70)
   print("PORTAGAS Scan started at:" + str(t1))
   if scantype=='1':
      ...
      for i in top_1000_ports:
          try:
              ...
              try:
               ...
               continue
              result = conn.connect_ex((IPAddr,i))
              if result==0 :
                print(str(i)+"\{}".format("tcp")+"\t{}\t".format("OPEN")+service)
              else :
                print(str(i)+"\{}".format("tcp")+"\t{}\t".format("CLOSED")+service)
              conn.close()
       ...
       try:
         ...
         for port in range(1,65535):
            ...
            if result==0 :
                print(str(port)+"\{}".format("tcp")+"\t{}\t".format("OPEN")+service)
            else :
               print(str(port)+"\{}".format("tcp")+"\t{}\t".format("CLOSED")+service)
            s.close()
       except KeyboardInterrupt:
             print("\n Exiting Program !!!!")
             exit()
      ...
      try:
        ...
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        socket.setdefaulttimeout(1)
        service=socket.getservbyport(prt)
        result = s.connect_ex((IPAddr,prt))
        if result==0 :
             print(str(prt)+"\{}".format("tcp")+"\t{}\t".format("OPEN")+service)
        else :
             print(str(prt)+"\{}".format("tcp")+"\t{}\t".format("CLOSED")+service)
        s.close()
      except KeyboardInterrupt:
             print("\n Exiting Program !!!!")
             exit()
   ...
   t2=datetime.now()
   scantime = t2-t1
   print("Scan Completed at "+str(t2))
   print("ELAPSED TIME : "+str(scantime))
except KeyboardInterrupt:
 print("\nSession Closed!!")
 exit()
print("-"*70)